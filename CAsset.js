...
class CAsset extends Asset {
  ...
  collectDependencies() {
    ...
    for (let i = this.cFiles.length - 1; i >= 0; i--) {
      this.addDependency("./" + this.cFiles[i], {
        includedInParent: true,
        name: getAbs(this.cFiles[i]),
      });
    }
  }
  ...
  async parse() {
    ...
    await exec("em++", args, { cwd: path.dirname(this.name) });
  ...
  async checkEmscripten() {
    ...
    try {
      ...
      throw new Error("Emscripten isn't installed!");
    }
    installed = true;
  }
  ...
  async generate() {
    ...
    return {
      js: (await fs.readFile(this.outPath, "utf8")).replace(
        new RegExp(name, "g"),
        publicURL + "/" + name
      ),
